...
( find ./${PLUGIN_NAME} -path *Resources/store && find ./${PLUGIN_NAME} -type d -name ".git" && find ./${PLUGIN_NAME} -name "build.sh"  && find ./${PLUGIN_NAME} -name ".gitignore" && find ./${PLUGIN_NAME} -name ".gitmodules" ) | xargs rm -r
